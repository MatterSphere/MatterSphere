...
namespace FWBS.OMS.Addin.Security.Business
{
    public class UserPermissions : IPermissions
    {
        ...
        internal UserPermissions(User user)
        {
            OMS = new ReportingServer("FWBS Limited 2005");
            current = user;
            SystemPolicy policy = new SystemPolicy();
            policy.GetPolicyByUserID(current.ID);
            permissions = policy.Permissions;
            if (policy.Type != "EXPLICITSYS")
                policyid = policy.ID;
            else
                policyid = "";
        }
        ...
        public string UpdatePermission()
        {
            ...
            paramlist[1] = OMS.Connection.AddParameter("userID", current.ID);
            ...
            return Convert.ToString(paramlist[0].Value).ToLowerInvariant();
        }
    }
}
