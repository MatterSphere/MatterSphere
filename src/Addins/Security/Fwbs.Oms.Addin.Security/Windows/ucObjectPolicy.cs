...
{
    public partial class ucObjectPolicy : FWBS.OMS.UI.Windows.Admin.ucEditBase2
    {
        private ObjectPolicy _policy = new ObjectPolicy();
        private readonly string _objectPolicyCodeLabel;
        private bool validated = true;

        public ucObjectPolicy()
        {
            InitializeComponent();
            ...
            _objectPolicyCodeLabel = Session.CurrentSession.Resources.GetResource("OBJPOLICYCODE", "Object Policy Code", "").Text + " : ";
        }
        ...
        protected override void LoadSingleItem(string Code)
        {
            ...
            labSelectedObject.Text = _objectPolicyCodeLabel + _policy.Type;
            tbcEdit.Buttons[0].Enabled = true;
            ShowPermissions("");
            ShowEditor();
            IsRemote.Checked = _policy.IsRemote;
            tbSave.Enabled = (_policy.Type.StartsWith("GLOBALOBJ") == false);
            this.IsDirty = false;
            SetInitialPermissionDescriptionColour();
        }
        ...
        protected override string SearchListName
        {
            get
            {
                return "SECOBJPOLICIES";
            }
        }

        protected override void Clone(string Code)
        {
            ObjectPolicy poly = new ObjectPolicy();
            string typecode = "";
            while (typecode == "")
            {
        ...
        protected override void NewData()
        {
            ...
            SetInitialPermissionDescriptionColour();
            labSelectedObject.Text = _objectPolicyCodeLabel + typecode;
            _policy.Type = typecode;
            txtName.Text = "";
            tbSave.Enabled = true;
            ShowPermissions("");
        ...
        protected override void DeleteData(string Code)
        {
            try
            {
                if (Code == ObjectPolicy.SystemSecurityCode)
                    throw new Exception("You cannot delete the Global Object Policy ...");
                _policy.GetPolicy(Code);
                _policy.Delete();
            }
            catch (Exception ex)
            {
                ErrorBox.Show(ex.InnerException ?? ex);
            }
            base.ShowList();
        }
        ...
        protected override bool UpdateData()
        {
            ...
            if (!CanSaveAfterPermissionCheck())
            {
                validated = false;
                return false;
            }
        ...
        {
            labSelectedObject.Text = _objectPolicyCodeLabel + txtName.Text;
            _policy.Name = txtName.Text;
            if (tbSave.Enabled) IsDirty = true;
        }
        ...
        private void txtActiveFilter_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        private void IsRemote_CheckedChanged(object sender, EventArgs e)
        {
            _policy.IsRemote = IsRemote.Checked;
            if (tbSave.Enabled) IsDirty = true;
        }
            ...
            if (base.IsDirty)
            {
                ...
                if (dr != DialogResult.Cancel)
                {
                    if(dr == DialogResult.No)
                        base.ShowList();
                    if (dr == DialogResult.Yes && validated)
                        base.ShowList();
                }
            }
            else
        ...
        private void SetInitialPermissionDescriptionColour()
        {
            foreach(Control control in pnlPermissions.Controls)
            {
                PermissionItem item = (PermissionItem)control;
                item.DescriptionColour = Color.FromArgb(0, 0, 0);
            }
        }
