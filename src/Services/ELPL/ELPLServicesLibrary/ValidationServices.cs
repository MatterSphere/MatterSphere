    ...
    static class ValidationServices
    {
        ...
        public static void ValidateXML(string xmlData, string xmlScemas)
        {
            try
            {
                ...
                string _fileLocation = Convert.ToString(new ApplicationSetting("ELPL", "", "", "XSDLocation").GetSetting("NotSet"));
        ...
        internal static string GetXSDForValidation(ELPLServiceBase serviceBase, string applicationId, ProcessActions action)
        {
            ELPLServices1 elplServices1 = (serviceBase as ELPLServices1) ?? new ELPLServices1(serviceBase.LoginDetails, serviceBase.TokenStorageProvider);

            var processVersion = elplServices1.GetSystemProcessVersion(applicationId, elplServices1);
            var processVersionCode = elplServices1.GetSystemProcessVersionReleaseCode(processVersion);
