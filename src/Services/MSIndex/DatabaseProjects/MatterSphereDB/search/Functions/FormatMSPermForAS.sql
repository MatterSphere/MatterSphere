	...
	BEGIN
	 ...
	 begin
		...
		begin
			...
			MERGE @T AS Target USING (
			SELECT 
					D.DocID AS ID,
					row_number() over (PARTITION BY D.DocID order by D.DocID ) + @maxnum as rn,
					coalesce(CAST(U.ID AS NVARCHAR(40)),CAST(RUGF.[UserGroupID] as nvarchar(40)))+
					CASE WHEN Substring ( PC.AllowMask , 9 , 1 ) & 128 = 128 THEN '(ALLOW)' ELSE 
					CASE WHEN Substring ( PC.DenyMask , 9 , 1 ) & 128 = 128 THEN '(DENY)' ELSE '(ALLOW)' END END AS UGDP
			...
			ON (Target.ID = Source.ID AND LEFT(Target.UGDP, 36) = LEFT(Source.UGDP, 36))
			WHEN NOT MATCHED BY Target THEN
				INSERT (ID, rn, UGDP)
				VALUES (Source.ID, Source.rn, Source.UGDP)
			WHEN MATCHED AND Target.UGDP <> Source.UGDP AND CHARINDEX('(DENY)', Source.UGDP) > 0 THEN
				UPDATE SET Target.UGDP = Source.UGDP
			;
		end
	  end
	end
