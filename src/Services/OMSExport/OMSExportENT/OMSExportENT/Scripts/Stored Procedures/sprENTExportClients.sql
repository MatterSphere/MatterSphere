/****** Object:  StoredProcedure [dbo].[sprENTExportClients]    Script Date: 07/27/2012 14:28:56 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sprENTExportClients]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sprENTExportClients]
GO



CREATE PROCEDURE [dbo].[sprENTExportClients]
AS

--CHECK IF A CUSTOM VERSION EXISTS - MUST RETURN SAME COLUMNS AS STANDARD VERSION
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sprENTExportClients_CUSTOM]') AND type in (N'P', N'PC'))
	EXEC [dbo].[sprENTExportClients_CUSTOM]
ELSE
BEGIN
	...
	SELECT 
		...
		, CLNO						AS CLNUM
		, (SELECT TOP 1 SPDATA FROM DBSPECIFICDATA WHERE spLookup = '__FDENTCLIMODEL')	AS MODEL -- CHECK
	...
	WHERE 
		-- only include clients that need exporting and are not yet been mapped in the integration table
		CL.CLNEEDEXPORT = 1 AND 
		MAPPEDCLIENT.EXTERNALID IS NULL
END 
