CREATE FUNCTION [dbo].[GetE3EMatterExportPostProcessesXML] ( @FILEID BIGINT , @MATTINDEX INT , @SPECIAL NVARCHAR ( 15 ) , @PROCESS_TYPE NCHAR (3) = N'E3E' )
...
SELECT
	...
	, @SEARCHREASON		= CASE --IF THE EXTERNAL ID IS NOT NULL, THEN THE MATTER HAS ALREADY GONE TO 3E
		WHEN DBFILE.FILEEXTLINKID IS NULL AND (@PROCESS_TYPE = N'E3E' OR (@PROCESS_TYPE = N'C3E' AND DBFILE.FILEEXTLINKTXTID IS NULL))
		THEN 'NewMatterExistingClient'
		ELSE 'NewPartyExistingMatter'
	END 
	, @SUBMITTER = CASE --IF -1, THEN THE MATTER HAS NOT GONE ACROSS ANYWAY.  IF 0 THEN THE MATTER IS NOT EXPORTER YET, SO GET THE USRADID FROM THE DBFILE.CREATED USER.  OTHERWISE SEND THROUGH THE SUSER_SNAME FROM SQL - ASSUMES WINDOWS AUTHENTICATION.
		WHEN ISNULL ( DBFILE.FILEEXTLINKID, 0 ) = -1 THEN 'ERROR'
		WHEN DBFILE.FILEEXTLINKID IS NULL AND (@PROCESS_TYPE = N'E3E' OR (@PROCESS_TYPE = N'C3E' AND DBFILE.FILEEXTLINKTXTID IS NULL)) THEN DBUSER.USRADID 
		ELSE SUSER_SNAME ()
	END
FROM
	DBFILE
...
IF ( SELECT COUNT ( * ) FROM DBFILE WHERE (FILEEXTLINKID IS NOT NULL OR (@PROCESS_TYPE = N'C3E' AND FILEEXTLINKTXTID IS NOT NULL)) AND CLID = @CLID  ) = 0
BEGIN
...
'
		...
		WHERE 
			...
			(ISNULL ( CONTEXTID , 0 ) > 0 OR			--WHERE THE ENTITY HAS ALREADY BEEN SENT ACROSS SUCCESSFULLY - IE WE HAVE AN ENTITY ID
				(@PROCESS_TYPE = N'C3E' AND CONTEXTTXTID IS NOT NULL)) AND
		...
		OPEN ASSOCIATE_LOOP
		FETCH NEXT FROM ASSOCIATE_LOOP INTO @ASSOCID , @CONTID , @CFTPARTYTYPE , @ENTITY , @ENTITYID , @COMPANYNAME , @FIRSTNAME , @MIDDLENAME , @LASTNAME , @CFTROLE , @CFTRELATIONSHIPCODE , @ASSOCEXTTXTID , @ASSOCORDER , @NOTES
...
'        <NBI:%ADD_EDIT%>
         <NBI:CftNewBizSearchName%POSITION_FOR_EDIT%>
          <NBI:Attributes>
           ...
           <NBI:Entity' + CASE WHEN @PROCESS_TYPE = N'C3E' THEN ' Id="' + CONVERT(NVARCHAR(21), @CONTID) + '" extId="' + ISNULL(@ENTITYID, '') + '">' ELSE '>' END
						+				ISNULL( @ENTITY, '' )			+ '</NBI:Entity>
