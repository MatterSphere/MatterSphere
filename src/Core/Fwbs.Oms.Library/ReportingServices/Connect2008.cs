...
namespace FWBS.OMS.ReportingServices
{
    ...
    /// </summary>
    internal class SSRSConnect2008 : ISSRSWS
	{
		...
		public SSRSConnect2008(bool ManageShared)
		{
		...
		public string[] UploadReport(string ReportToUpload, string DestinationPath, out string reportName)
		{
			...
			stream.Close();
            Warning[] wwarnings;
	        wwarnings = rs.CreateReport(reportName, DestinationPath, true, definition,null);
			if (wwarnings != null)
			{
		...
		public DataTable Extensions(ExtensionsOptions options)
		{
			...
			{
                var items = rs.ListExtensions(ExtensionTypeEnum.Delivery);
                foreach (var i in items)
                    if (i.Name == "Report Server Email")
                        extensionParams = ProcessExtensionsByPermissions(rs.GetExtensionSettings("Report Server Email"));
                if (extensionParams == null)
                    throw new OMSException2("EMAILDELERR","Email Delivery is not installed or configured");
		...
		public string ReportWebUrlPath
		{
			get
			{
                return SSRSConnect.Server + "/Pages/ReportViewer.aspx?";
			}
		}
