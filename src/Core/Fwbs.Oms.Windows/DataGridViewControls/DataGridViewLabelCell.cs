...
namespace FWBS.OMS.UI.Windows
{
    public class DataGridViewLabelCell : DataGridViewTextBoxCell
    {
        ...
        private void PaintImage(Graphics graphics, Rectangle cellBounds, Padding cellPadding, int rowIndex)
        {
            ...
            {
                rect = new Rectangle(cellBounds.Right - cellPadding.Right, cellBounds.Top + (cellBounds.Height - imageSize) / 2, imageSize, imageSize);
            }
            else
            {
                rect = new Rectangle(cellBounds.Left + cellPadding.Left - imageSize, cellBounds.Top + (cellBounds.Height - imageSize) / 2, imageSize, imageSize);
            }
        ...
        private DataGridViewLabelColumn OwningLabelColumn
        {
            get
            {
                return (DataGridViewLabelColumn)OwningColumn;
            }
        }
