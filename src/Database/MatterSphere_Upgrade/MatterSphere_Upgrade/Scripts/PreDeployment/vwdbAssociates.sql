IF EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[config].[vwdbAssociates]'))
	DROP VIEW [config].[vwdbAssociates]
GO
...
DECLARE @TABLE NVARCHAR(150) = 'dbAssociates'
	, @VIEW_SCHEMA NVARCHAR(150) = 'config'
	, @VIEW_NAME NVARCHAR(150) = 'vwdbAssociates'
