...
WHERE ut.name = 'uAddressLine' AND ut.schema_id = SCHEMA_ID('dbo') AND ut.max_length = 100 AND st.name = 'nvarchar'
...
' AS c
	...
	SET @Sql = (SELECT N'ALTER TABLE ' + QUOTENAME(s.name) + '.' + QUOTENAME(t.name) + N' ALTER COLUMN ' + QUOTENAME(c.name) + N' NVARCHAR(64)
'
	FROM sys.all_columns c
		INNER JOIN sys.tables t ON c.object_id = t.object_id
		INNER JOIN sys.schemas s ON t.schema_id = s.schema_id
...
EXEC sp_refreshview ''' + QUOTENAME(s.name) + N'.' + QUOTENAME(o.name) + ''''
	...
	BEGIN
		...
		SET @Sql = (SELECT N'ALTER TABLE ' + QUOTENAME(s.name) + '.' + QUOTENAME(t.name) + N' ALTER COLUMN ' + QUOTENAME(c.name) + N' uAddressLine
'
		FROM sys.all_columns c
			INNER JOIN sys.tables t ON c.object_id = t.object_id
			INNER JOIN sys.schemas s ON t.schema_id = s.schema_id
	...
	BEGIN
		...
		BEGIN
			SET @Sql = (SELECT REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(Code, '[dbo].[uAddressLine]', 'NVARCHAR(64)'), '[dbo].uAddressLine', 'NVARCHAR(64)'), 'dbo.[uAddressLine]', 'NVARCHAR(64)'), 'dbo.uAddressLine', 'NVARCHAR(64)') , 'uAddressLine', 'NVARCHAR(64)') FROM @Proc WHERE oId = @oID)
	...
	BEGIN
		...
		EXEC sp_executesql N'DROP TYPE [dbo].[uAddressLine]'
		EXEC sp_executesql N'CREATE TYPE [dbo].[uAddressLine] FROM [NVARCHAR](64) NULL'
