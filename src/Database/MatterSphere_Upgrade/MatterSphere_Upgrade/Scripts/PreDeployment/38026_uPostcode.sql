...
WHERE ut.name = 'uPostcode' AND ut.schema_id = SCHEMA_ID('dbo') AND ut.max_length = 30 AND st.name = 'nvarchar'
...
' AS c
	...
	SET @Sql = (SELECT N'ALTER TABLE ' + QUOTENAME(s.name) + '.' + QUOTENAME(t.name) + N' ALTER COLUMN ' + QUOTENAME(c.name) + N' NVARCHAR(20)
'
	FROM sys.all_columns c
		INNER JOIN sys.tables t ON c.object_id = t.object_id
		INNER JOIN sys.schemas s ON t.schema_id = s.schema_id
...
EXEC sp_refreshview ''' + QUOTENAME(s.name) + N'.' + QUOTENAME(o.name) + ''''
	...
	BEGIN
		...
		SET @Sql = (SELECT N'ALTER TABLE ' + QUOTENAME(s.name) + '.' + QUOTENAME(t.name) + N' ALTER COLUMN ' + QUOTENAME(c.name) + N' uPostcode
'
		FROM sys.all_columns c
			INNER JOIN sys.tables t ON c.object_id = t.object_id
			INNER JOIN sys.schemas s ON t.schema_id = s.schema_id
	...
	BEGIN
		...
		BEGIN
			SET @Sql = (SELECT REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(Code, '[dbo].[uPostcode]', 'NVARCHAR(20)'), '[dbo].uPostcode', 'NVARCHAR(20)'), 'dbo.[uPostcode]', 'NVARCHAR(20)'), 'dbo.uPostcode', 'NVARCHAR(20)') , 'uPostcode', 'NVARCHAR(20)') FROM @Proc WHERE oId = @oID)
	...
	BEGIN
		...
		EXEC sp_executesql N'DROP TYPE [dbo].[uPostcode]'
		EXEC sp_executesql N'CREATE TYPE [dbo].[uPostcode] FROM [NVARCHAR](20) NULL'
...
BEGIN
	SELECT @user_type_id = ut.user_type_id
	FROM sys.types ut INNER JOIN sys.types st  ON st.user_type_id = ut.system_type_id
	WHERE ut.name = 'uPostcode' AND ut.schema_id = SCHEMA_ID('dbo') AND st.name = 'nvarchar'
