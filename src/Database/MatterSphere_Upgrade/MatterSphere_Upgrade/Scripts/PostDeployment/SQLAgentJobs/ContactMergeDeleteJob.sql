Print 'SQLAgentJobs\ContactMergeDeleteJob.sql'
...
BEGIN
	...
	EXEC msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'dbContactLinks', 
			...
			@retry_interval=0, 
			@os_run_priority=0, @subsystem=N'TSQL', 
			@command=N'EXECUTE dbo.ContactMergeDelete_dbContactLinks', 
			@database_name=@db_name, 
			@flags=0
...
BEGIN
	...
	DECLARE @StepID Int, @ScheduleID Int
	...
	BEGIN
		...
		EXEC msdb.dbo.sp_delete_jobstep @job_id = @jobId_Exist, @step_id = @StepID	
	END
	IF EXISTS (SELECT step_id FROM msdb.dbo.sysjobsteps WHERE job_id = @jobId_Exist and Step_name = 'dbContactLinks'
	                                                      and command = 'EXECUTE dbo.ContactMergeDelete_dbContactIndividual')
	BEGIN
		Select @StepID = step_id FROM msdb.dbo.sysjobsteps WHERE job_id = @jobId_Exist and Step_name = 'dbContactLinks'
		EXEC msdb.dbo.sp_update_jobstep @job_id=@jobId_Exist, @step_id=@StepID, 
		@command=N'EXECUTE dbo.ContactMergeDelete_dbContactLinks'
	END

	SELECT @ScheduleID = JS.schedule_id
	FROM msdb.dbo.sysjobschedules JS INNER JOIN msdb.dbo.sysschedules S ON S.schedule_id = JS.schedule_id
	WHERE JS.job_id = @jobId_Exist and S.active_end_date = '20100512'

	IF @ScheduleID IS NOT NULL
	BEGIN
		EXEC msdb.dbo.sp_update_schedule @schedule_id = @ScheduleID, @active_end_date = '99991231', @enabled = 1
	END
END
GO
