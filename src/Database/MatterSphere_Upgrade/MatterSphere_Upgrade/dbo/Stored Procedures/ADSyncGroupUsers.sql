...
BEGIN TRANSACTION
	...
	SELECT @groupID , Y.ID
	FROM
	(SELECT UserNTName FROM @userTable EXCEPT SELECT NTLogin FROM [item].[User] U JOIN [relationship].[Group_User] R ON R.UserID = U.ID WHERE GroupID = @groupID ) Z
	...
	( 
		...
		(
			...
			EXCEPT SELECT UserNTName FROM @userTable 
