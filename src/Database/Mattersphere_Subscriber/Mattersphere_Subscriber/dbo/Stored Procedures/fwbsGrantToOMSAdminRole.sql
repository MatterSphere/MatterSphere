...
CREATE PROCEDURE [dbo].[fwbsGrantToOMSAdminRole]
AS
IF NOT EXISTS ( SELECT [name] FROM sysusers WHERE [name] = 'OMSAdminRole' )
BEGIN
	RAISERROR ( 'The role OMSAdminRole does not exist' , 10 , 1)
	RETURN
END
...
BEGIN
	...
	--BEGIN 
		--SET @sql = 'GRANT execute on ' + @Procedure + ' to OMSAdminRole'
	...
	--WHILE @@fetch_status = 0
		--BEGIN 
		--SET @sql = 'GRANT SELECT on ' + @table + ' to OMSAdminRole'
		--EXEC sp_executesql @sql
		--FETCH NEXT FROM access_cursor INTO @table
	--END
