...
CREATE PROCEDURE [dbo].[MatterMergeDelete_dbFileManagementAppData]
AS
SET NOCOUNT ON
...
BEGIN TRANSACTION
	...
	DELETE dbo.dbFileManagementAppData OUTPUT deleted.fileID INTO @logTable WHERE fileID = @fileID
	
	IF @@ROWCOUNT > 0
	BEGIN
		SET @logXML = ( SELECT (SELECT ID as fileID FROM @logTable FOR XML PATH(''), TYPE ) FOR XML PATH('dbFileManagementAppData'))
		IF (@logString = '<log/>')
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[MatterMergeDelete_dbFileManagementAppData] TO [OMSRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[MatterMergeDelete_dbFileManagementAppData] TO [OMSAdminRole]
    AS [dbo];
