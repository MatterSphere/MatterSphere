...
CREATE PROCEDURE [dbo].[ClientDelete_dbContactEmails]
AS
SET NOCOUNT ON

DECLARE @clID bigint, @contactID bigint, @logID int, @logXML xml, @logString nvarchar(MAX)
DECLARE @logTable table ( ID bigint , Email varchar(150)) 
...
BEGIN TRANSACTION
	...
	DELETE dbo.dbContactEmails OUTPUT deleted.contID, deleted.contEmail INTO @logTable WHERE contID = @contactID
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[ClientDelete_dbContactEmails] TO [OMSRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[ClientDelete_dbContactEmails] TO [OMSAdminRole]
    AS [dbo];
