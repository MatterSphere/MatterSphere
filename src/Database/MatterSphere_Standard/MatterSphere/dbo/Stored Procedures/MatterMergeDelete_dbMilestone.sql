...
CREATE PROCEDURE [dbo].[MatterMergeDelete_dbMilestone]
AS
SET NOCOUNT ON
...
BEGIN TRANSACTION
	...
	BEGIN
		DELETE dbo.dbMilestone
		OUTPUT deleted.msID INTO @logTable
		WHERE fileID = @fileID
	END
	ELSE
	BEGIN
		UPDATE dbo.dbMilestone SET fileID = @newFileID 
		OUTPUT deleted.msID INTO @logTable
		WHERE fileID = @fileID
	END
	...
	IF @@ROWCOUNT > 0
	BEGIN
		SET @logXML = ( SELECT (SELECT ID as msID FROM @logTable FOR XML PATH(''), TYPE ) FOR XML PATH('dbMilestone'))
		IF (@logString = '<log/>')
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[MatterMergeDelete_dbMilestone] TO [OMSRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[MatterMergeDelete_dbMilestone] TO [OMSAdminRole]
    AS [dbo];
