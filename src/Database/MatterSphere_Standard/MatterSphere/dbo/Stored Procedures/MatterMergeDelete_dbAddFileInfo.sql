...
CREATE PROCEDURE [dbo].[MatterMergeDelete_dbAddFileInfo]
AS
SET NOCOUNT ON
...
BEGIN TRANSACTION
	...
	DELETE dbo.dbAddFileInfo OUTPUT deleted.fileID INTO @logTable WHERE fileID = @fileID
	
	IF @@ROWCOUNT > 0
	BEGIN
		SET @logXML = ( SELECT (SELECT ID as fileID FROM @logTable FOR XML PATH(''), TYPE ) FOR XML PATH('dbAddFileInfo'))
		IF (@logString = '<log/>')
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[MatterMergeDelete_dbAddFileInfo] TO [OMSRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[MatterMergeDelete_dbAddFileInfo] TO [OMSAdminRole]
    AS [dbo];
