...
-- Description:	<Return all tabs for all Matter types>
-- =============================================
CREATE PROCEDURE [dbo].[sprOMSFileTypeTabList] 
...
insert into @t1
select typeCode, cast(typeXML as xml) from dbFileType 

SELECT typeCode
	, COALESCE(CL1.cdDesc, '~' + NULLIF(res.typeCode, '') + '~') as [File Type]
...
FROM (
	...
	LEFT JOIN dbo.GetCodeLookupDescription ( 'FILETYPE', '' ) CL1 ON CL1.[cdCode] =  res.typeCode
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sprOMSFileTypeTabList] TO [OMSRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sprOMSFileTypeTabList] TO [OMSAdminRole]
    AS [dbo];
