...
CREATE PROCEDURE [dbo].[ContactMergeDelete_dbContactCompany]
AS
SET NOCOUNT ON
...
BEGIN TRANSACTION
	...
	DELETE dbo.dbContactCompany
	OUTPUT deleted.contID INTO @logTable
	WHERE contID = @contID 
	
	IF @@ROWCOUNT > 0
	BEGIN
		SET @logXML = ( SELECT (SELECT ID as 'contID' FROM @logTable FOR XML PATH(''), TYPE ) FOR XML PATH('dbContactCompany'))
		IF (@logString = '<log/>')
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[ContactMergeDelete_dbContactCompany] TO [OMSRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[ContactMergeDelete_dbContactCompany] TO [OMSAdminRole]
    AS [dbo];
