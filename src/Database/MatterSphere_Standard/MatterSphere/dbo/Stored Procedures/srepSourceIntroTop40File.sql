...
CREATE PROCEDURE [dbo].[srepSourceIntroTop40File]
...
set @select = N'SELECT     dbo.dbCodeLookup.cdDesc, dbo.dbContact.contName, dbo.dbFile.Created, 
...
FROM         dbo.dbFile INNER JOIN
                      dbo.dbCodeLookup ON dbo.dbFile.fileSource = dbo.dbCodeLookup.cdCode AND dbo.dbcodelookup.CDTYPE = ''SOURCE'' INNER JOIN
		      dbo.dbclient on dbo.dbfile.clid = dbo.dbclient.clid inner join
                      ...
                      dbo.dbFeeEarner ON dbo.DbFile.fileprincipleID = dbo.dbFeeEarner.feeusrID INNER JOIN
                      dbo.dbUser ON dbo.dbFeeEarner.feeusrID = dbo.dbUser.usrID '

set @groupby = N' GROUP BY dbo.dbCodeLookup.cdDesc, dbo.dbContact.contName, dbo.dbfile.Created, dbo.dbCodeLookup.cdType, dbo.dbClient.clPreClient, 
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[srepSourceIntroTop40File] TO [OMSRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[srepSourceIntroTop40File] TO [OMSAdminRole]
    AS [dbo];
