...
-- Description:	<Return all tabs for all Fee Earner types>
-- =============================================
CREATE PROCEDURE [dbo].[sprOMSFeeEarnerTypeTabList] 
...
insert into @t1
select typeCode, cast(typeXML as xml) from dbFeeEarnerType 

SELECT typeCode
	, COALESCE(CL1.cdDesc, '~' + NULLIF(res.typeCode, '') + '~') as [Fee Earner Type]
...
FROM (
	...
	LEFT JOIN dbo.GetCodeLookupDescription ( 'FEEEARNERTYPE', '' ) CL1 ON CL1.[cdCode] =  res.typeCode
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sprOMSFeeEarnerTypeTabList] TO [OMSRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sprOMSFeeEarnerTypeTabList] TO [OMSAdminRole]
    AS [dbo];
