...
-- Description:	<Return all tabs for all Command Centre types>
-- =============================================
CREATE PROCEDURE [dbo].[sprOMSCommandCentreTypeTabList] 
...
insert into @t1
select typeCode, cast(typeXML as xml) from dbCommandCentreType 

SELECT typeCode
	, COALESCE(CL1.cdDesc, '~' + NULLIF(res.typeCode, '') + '~') as [Command Centre Type]
...
FROM (
	...
	LEFT JOIN dbo.GetCodeLookupDescription ( 'CENTRETYPE', '' ) CL1 ON CL1.[cdCode] =  res.typeCode
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sprOMSCommandCentreTypeTabList] TO [OMSRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sprOMSCommandCentreTypeTabList] TO [OMSAdminRole]
    AS [dbo];
