...
CREATE PROCEDURE [dbo].[ContactMergeDelete_dbContactIndividual]
AS
SET NOCOUNT ON
...
BEGIN TRANSACTION
	...
	DELETE dbo.dbContactIndividual
	OUTPUT deleted.contID INTO @logTable
	WHERE contID = @contID 
	
	IF @@ROWCOUNT > 0
	BEGIN
		SET @logXML = ( SELECT (SELECT ID as 'contID' FROM @logTable FOR XML PATH(''), TYPE ) FOR XML PATH('dbContactIndividual'))
		IF (@logString = '<log/>')
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[ContactMergeDelete_dbContactIndividual] TO [OMSRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[ContactMergeDelete_dbContactIndividual] TO [OMSAdminRole]
    AS [dbo];
