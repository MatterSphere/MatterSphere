...
CREATE PROCEDURE [dbo].[ContactMergeDelete_dbContactReferral]
AS
SET NOCOUNT ON

DECLARE @contID bigint, @newContID bigint, @logID int, @logXML xml, @logString nvarchar(MAX)
DECLARE @logTable table ( ID bigint, contID bigint ) 
...
BEGIN TRANSACTION
	...
	DELETE dbo.dbContactReferral
	OUTPUT deleted.refID, deleted.contID INTO @logTable
	WHERE contID = @contID OR refContact = @contID
	
	IF @@ROWCOUNT > 0
	BEGIN
		SET @logXML = ( SELECT (SELECT ID as '@refID', contID as'@contID' FROM @logTable FOR XML PATH('row'), TYPE ) FOR XML PATH('dbContactReferral'))
		IF (@logString = '<log/>')
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[ContactMergeDelete_dbContactReferral] TO [OMSRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[ContactMergeDelete_dbContactReferral] TO [OMSAdminRole]
    AS [dbo];
