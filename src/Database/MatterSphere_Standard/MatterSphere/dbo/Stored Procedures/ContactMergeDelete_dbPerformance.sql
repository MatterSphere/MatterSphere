...
CREATE PROCEDURE [dbo].[ContactMergeDelete_dbPerformance]
AS
SET NOCOUNT ON
...
BEGIN TRANSACTION
	...
	DELETE dbo.dbPerformance
	OUTPUT deleted.contID INTO @logTable
	WHERE contID = @contID 
	
	IF @@ROWCOUNT > 0
	BEGIN
		SET @logXML = ( SELECT (SELECT ID as contID FROM @logTable FOR XML PATH(''), TYPE ) FOR XML PATH('dbPerformance'))
		IF (@logString = '<log/>')
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[ContactMergeDelete_dbPerformance] TO [OMSRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[ContactMergeDelete_dbPerformance] TO [OMSAdminRole]
    AS [dbo];
