...
-- Description:	<Return all tabs for all Client types>
-- =============================================
CREATE PROCEDURE [dbo].[sprOMSClientTypeTabList] 
...
insert into @t1
select typeCode, cast(typeXML as xml) from dbClientType 

SELECT typeCode
	, COALESCE(CL1.cdDesc, '~' + NULLIF(res.typeCode, '') + '~') as [Client Type]
...
FROM (
	...
	LEFT JOIN dbo.GetCodeLookupDescription ( 'CLTYPE', '' ) CL1 ON CL1.[cdCode] =  res.typeCode
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sprOMSClientTypeTabList] TO [OMSRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sprOMSClientTypeTabList] TO [OMSAdminRole]
    AS [dbo];
