...
-- Description:	<Return all tabs for all Contact types>
-- =============================================
CREATE PROCEDURE [dbo].[sprOMSContactTypeTabList] 
...
insert into @t1
select typeCode, cast(typeXML as xml) from dbContactType 

SELECT typeCode
	, COALESCE(CL1.cdDesc, '~' + NULLIF(res.typeCode, '') + '~') as [Contact Type]
...
FROM (
	...
	LEFT JOIN dbo.GetCodeLookupDescription ( 'CONTTYPE', '' ) CL1 ON CL1.[cdCode] =  res.typeCode
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sprOMSContactTypeTabList] TO [OMSRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sprOMSContactTypeTabList] TO [OMSAdminRole]
    AS [dbo];
