...
CREATE PROCEDURE [config].[GetContactPermissions]
	@objectID bigint , 
	@children bit,
	@status bit = 0 ,
	@User nvarchar(200)
...
BEGIN
	...
	CROSS JOIN
	[relationship].[UserGroup_Contact] Y
	JOIN [config].[ObjectPolicy] OP ON OP.ID = Y.PolicyID AND COALESCE (OP.IsRemote,0) = @RemotePolicy
	WHERE X.[SecurableType] IS NOT NULL AND X.[Permission] IS NOT NULL AND Y.[ContactID] = @objectID
	...
	JOIN
		[relationship].[UserGroup_Contact] UG ON UG.[PolicyID] = OP.[ID] CROSS APPLY [config].[MaskToPermissions] ( OP.[AllowMask] , OP.[DenyMask] ) MTP
	JOIN
		[config].[GetUserAndGroupMembershipNT] (@USER) GM ON GM.[ID] = UG.[UserGroupID]
	WHERE
		UG.[ContactID] = @objectID 
	AND
		CASE WHEN @children = 1 THEN MTP.[SecurableType] ELSE @securableType END = MTP.[SecurableType] 
	GROUP BY
		MTP.[PermissionCode] , MTP.[SecurableType] , UG.[ContactID] , MTP.[NodeLevel]
	) A
	ORDER BY
		A.[NodeLevel]
	RETURN
...
GO
GRANT EXECUTE
    ON OBJECT::[config].[GetContactPermissions] TO [OMSRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[config].[GetContactPermissions] TO [OMSAdminRole]
    AS [dbo];
