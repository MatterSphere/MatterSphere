...
CREATE PROCEDURE [config].[GetClientPermissions]
...
BEGIN
	...
	CROSS JOIN
	[relationship].[UserGroup_Client] Y
	JOIN [config].[ObjectPolicy] OP ON OP.ID = Y.PolicyID AND COALESCE (OP.IsRemote,0) = @RemotePolicy
	WHERE X.[SecurableType] IS NOT NULL AND X.[Permission] IS NOT NULL AND Y.[ClientID] = @objectID
	...
	JOIN
		[relationship].[UserGroup_Client] UG ON UG.[PolicyID] = OP.[ID] CROSS APPLY [config].[MaskToPermissions] ( OP.[AllowMask] , OP.[DenyMask] ) MTP
	JOIN
		[config].[GetUserAndGroupMembershipNT] (@USER) GM ON GM.[ID] = UG.[UserGroupID]
	WHERE
		UG.[ClientID] = @objectID 
	AND
		CASE WHEN @children = 1 THEN MTP.[SecurableType] ELSE @securableType END = MTP.[SecurableType] 
	GROUP BY
		MTP.[PermissionCode] , MTP.[SecurableType] , UG.[ClientID] , MTP.[NodeLevel]
	) A
	ORDER BY
		A.[NodeLevel]
	RETURN
...
GO
GRANT EXECUTE
    ON OBJECT::[config].[GetClientPermissions] TO [OMSRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[config].[GetClientPermissions] TO [OMSAdminRole]
    AS [dbo];
