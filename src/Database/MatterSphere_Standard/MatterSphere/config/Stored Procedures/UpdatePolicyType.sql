...
CREATE PROCEDURE [config].[UpdatePolicyType]
...
BEGIN TRANSACTION
	...
	VALUES ( 'Security Policy Updated' , @policyTypeCode , @canEdit , @canDelete , @getdate , @createdBy , @isSystemPolicy , @includeInFilters )
COMMIT TRANSACTION
END TRY
BEGIN CATCH
	IF @@Trancount <> 0
...
GO
GRANT EXECUTE
    ON OBJECT::[config].[UpdatePolicyType] TO [OMSRole]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[config].[UpdatePolicyType] TO [OMSAdminRole]
    AS [dbo];
